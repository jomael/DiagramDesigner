...
type
  ...
  TGroupObject = class(TBaseObject)
    ...
    public
      ...
      constructor CreateFromSelection(Objects: TDiagramLayer);
...
constructor TGroupObject.CreateFromSelection(Objects: TDiagramLayer);
var
  I : Integer;
  Stream : TMemStream;
...
begin
  ...
  try
    Objects.SaveSelected(Stream); // Use stream and SaveSelected to keep links
    Stream.Position:=0;
    TempLayer:=TDiagramLayer.Create;
    TempLayer.LoadSelected(Stream);
  finally
    Stream.Free;
  end;
  for I:=0 to TempLayer.Count-1 do
  begin
    Group.Add(TempLayer.Objects[I]);
    TempLayer.Objects[I]:=nil;
  end;
  TempLayer.Free;
...
var
  I : Integer;
begin
  Result:=TGroupObject.Create;
  Result.Assign(Self);
  for I:=0 to Group.Count-1 do
    TGroupObject(Result).Group.Objects[TGroupObject(Result).Group.Add(Group.Objects[I].CreateCopy)].Selected:=True;
end;
...
begin                                                 
  ...
  if LocalCanvasInfo.DrawMode=dmEditing then LocalCanvasInfo.DrawMode:=dmPreview;
...
// TPolygonObject
//==============================================================================================================================
function TPolygonObject.CreateCopy: TBaseObject;
begin
  Result:=TPolygonObject.Create;
  Result.Assign(Self);
end;

procedure TPolygonObject.Rotate(const Angle: Double; FlipLR,FlipUD: Boolean; const Center: TPoint);
var
  P : TFloatPoint;
  I : Integer;
begin
  ...
  begin
    ...
    P:=VectorSubtract(RotatePoint(P,FloatPoint(Center),Angle),P);
...
begin
  Result:=(inherited ValidProperties)-[opTextYAlign,opMargin]+[opCustomLinks,opBlockAlignOnly];
end;

function TPolygonObject.GetProperty(Index: TObjectProperty): Integer;
begin
  case Index of
    opCustomLinks : Result:=Integer(@Links);
    else Result:=inherited GetProperty(Index);
