...
type
  ...
  public
    { Public declarations }
    class procedure Execute(Formats: string; const IconIndex: array of Integer; SelectAll: Boolean=False; SkipButton: Boolean=False);
  end;
...
class procedure TFormatAssociateForm.Execute(Formats: string; const IconIndex: array of Integer; SelectAll,SkipButton: Boolean);
var
  ...
  try
    ...
    begin
      ...
      begin
        ...
        P:=2;
        while (P<Length(Str)) and not (Str[P] in [',',';',' ',')']) do Inc(P);
        RegisterFileFormat(Copy(Str,1,P-1),Application.Title,Description,'',Tag);
      end
      else // Unassociate
      begin
        Str:=Caption;
