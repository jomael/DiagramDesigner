...
begin
	...
	if (FScreenScale <= 0) then
	begin
		FScreenScale := 96 / DesignerDPI;
	end;
...
begin
	...
	if ProjectIsTranslated then
	begin
		DefaultDefines.Reset();
	end;
	...
	end;		// for i - ActionManager.Actions[]
...
begin
	NewFile := FileName;
	if (not(Assigned(Sender)) or (CloseQuery() and OpenPictureDialog(NewFile, rsDiagramFileFilter))) then
	begin
...
begin
	...
	begin
		if (ExecuteFile(ParamStr(0), '"' + NewFile + '"') <= 32) then
		begin
			RaiseLastOSError();
		end;
	end;
...
begin
	...
	if (
		SaveFileDialog(ExportName, BitmapLoaders.GetSaveFilter) and
		FloatQuery(ItemCaption(ExportAction), rsExportFileDPI, ExportDPI, 64, 2400)
	) then
	begin
		try
			Screen.Cursor := crHourGlass;
			...
			try
				...
				begin
					...
					Bitmap.New(
						Round(GetActivePage.Width*ExportCanvasInfo.Scale.X),
...
begin
	...
	begin
		...
		begin
			...
			begin
				RecentFileList.Objects[I].Free();
			end;
			RecentFileList.Delete(I);
		end
		else if not(Assigned(RecentFileList.Objects[I])) then
		begin
...
begin
	...
	begin
		if Assigned(RecentFileList.Objects[I]) then
		begin
			if (RecentFileList[I] = '') then
			begin
				Exists := False
			end
			else if (RecentFileList[I][1] <> '\') and (TDriveType(GetDriveType(PChar(Copy(RecentFileList[I], 1, 3)))) in [dtFixed, dtRAM]) then
			begin
...
begin
	...
	if (I >= 0) then
	begin
		if (RecentFileList[I] = FileName) then
		begin
			ReloadActionExecute(nil);
		end
...
begin
	...
	try
		...
		begin
			...
			begin
				AutoOrientation := poLandscape;
			end
			else
			begin
				AutoOrientation := Printer.Orientation;
			end;
		end;
...
begin
	if Assigned(TemplateObject) then
	begin
...
begin
	...
	begin
		...
		begin
			SelectAll();
			if not(Assigned(ActiveObject)) then ActiveObject := Objects[Count - 1];
			UpdateDrawing();
		end;
	end;
end;
...
begin
	if not(Assigned(ActiveObject)) then Exit;
...
var
	...
	Format: Word;
begin
	if not(Assigned(Clipboard)) then Exit;
	DataHandle := 0;
	...
	begin
		SetClipboardData(Msg.Format, DataHandle);
	end;
end;
...
procedure TMainForm.PasteActionExecute(Sender: TObject);
	...
	begin
		...
		Data := GlobalLock(DataHandle);
		if not(Assigned(Data)) then RaiseLastOSError();
		try
			Size := Data^;
			Inc(Data);
			...
			try
				...
				begin
					...
					with Options do
					begin
						if ((Grid.X > 1) or (Grid.Y > 1)) then // Move pasted objects one grid step
	...
	begin
		...
		ReplaceChar(Text, #13, '\');
		ReplaceChar(Text, #10, 'n');		// TODO: what about other escaped sequences?
		ActiveObject := TTextObject.CreateNew(Text);
		with CanvasFrame.BitmapCanvas do
		begin
			Font := DrawPanel.Font;
			ActiveObject.Position := Bounds(
				FloorInt(MousePos.X, Options.Grid.X),
				FloorInt(MousePos.Y, Options.Grid.Y),
...
var
	...
	Metafile: TMetafile;
begin
	if (not(Assigned(Sender)) or OpenPictureDialog(PicturePath, rsKnownImageFormats + ' (' + BitmapLoaders.GetLoadFilter)) then
	try
		Screen.Cursor := crHourGlass;
		...
		begin
			...
			try
				...
				ActiveObject.Position := Bounds(MousePos.X, MousePos.Y, Image.Width * DesignerDPI div Screen.PixelsPerInch, Image.Height * DesignerDPI div Screen.PixelsPerInch);
			finally
				Image.Free();
...
begin
	...
	if not(IntegerQuery(Caption, rsRelativePageNumber1PreviousPage, RelativePageIndex, -ActivePageIndex, Diagram.Count - ActivePageIndex - 1, 200)) then Exit;
	LayerIndex := 1;
	if not(IntegerQuery(Caption, rsLayerNumber, LayerIndex, 1, Diagram.Pages[ActivePageIndex + RelativePageIndex].Count, 140)) then Exit;
...
begin
	...
	begin
		...
		CanvasInfo.Offset := Point(-ScrollBarX.Position, ScrollBarY.Position);
		...
		begin
			...
			// Draw grid:
			if ((Options.ShowGrid and (Options.Grid.X * CanvasInfo.Scale.X >= 6) and (Options.Grid.Y * CanvasInfo.Scale.Y >= 6))) then
			begin
...
begin
	...
	if Assigned(TemplateObject) then
	begin
		TemplatePropertiesAction.Caption := TemplateObject.Name;
	end
	else
	begin
		TemplatePropertiesAction.Caption := TemplatePropertiesAction.Hint;
	end;
end;
...
begin
	...
	begin
		...
		begin
			if Assigned(ActiveObject) then
			begin
				TreeView.Selected := TTreeNode(ActiveObject.TreeNode);
			end
			else
			begin
				TreeView.Selected := nil;
			...
			begin
				...
				begin
					...
					begin
						...
						begin
							Offset := GetActiveLayer().MoveSelected(MousePos.X - LastMousePos.X, MousePos.Y - LastMousePos.Y, ActiveObjectHandle, NoGrid, Shift);
						end
						else // Snap to grid
						begin
							Offset := GetActiveLayer.MoveSelected(MousePos.X-LastMousePos.X, MousePos.Y-LastMousePos.Y, ActiveObjectHandle, Options.Grid, Shift);
						end;
					end;
...
begin
	case Key of
		...
		Word('1')..Word('9'):
		begin
			if ((ssCtrl in Shift) and (Key - Word('1') < Diagram.Count)) then
			begin
		...
		Word('0'):
		begin
			if ((ssCtrl in Shift) and (Diagram.Count >= 10)) then
			begin
		...
		VK_NEXT:
		begin
			if (ActivePageIndex < Diagram.Count - 1) then
			begin
		...
		begin
			if (ssCtrl in Shift) then
			begin
				ActivePageIndex := Diagram.Count - 1;
			end;
		end;
		...
		begin
			...
			begin
				with ScrollBarY do Position := Position + SmallChange;
			end;
		end;
		...
		begin
			...
			begin
				with ScrollBarX do Position := Position + SmallChange;
			end;
		end;
...
begin
	ChangeDefault := not(Assigned(ActiveObject));
	...
	if (Assigned(Sender) and (Abs(Integer(Sender)) < 1000)) then
	begin
...
begin
	...
	begin
		Modified := TPropertyEditorForm.Execute(GetActiveLayer, Options);
	end
	else if (GetActiveLayer().SelectCount = 1) then
	begin
		Modified := TPropertyEditorForm.Execute(ActiveObject, Options, false);
	end
	else
	begin
		Node := TreeView.Selected;
		Modified := TPropertyEditorForm.Execute(TBaseObject(Node.Data), Options, Assigned(Node.Parent));
		if (Modified) then
...
begin
	ExecuteFile(
		'http://meesoft.logicnet.dk/DiagramDesigner/CheckForUpdates.php?Version=' + 
