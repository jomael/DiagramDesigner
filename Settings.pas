...
begin
  ...
  begin
    if Setup.ReadInteger('TopLeft',Integer(Pos)) and (Pos.X<Screen.Width) and (Pos.Y<Screen.Height) then
    begin
      Form.Left:=Pos.X;
      Form.Top:=Pos.Y;
      Result:=True;
    ...
    if (Form.Left+Form.Width)<0 then Form.Left:=0;
    if (Form.Top+Form.Height)<0 then Form.Top:=0;
  end;
end;
