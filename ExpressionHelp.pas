...
begin
  ...
  try
    ...
    PM:=SetPrecisionMode(pmExtended);
    try
...
begin
  Result:=TExpressionEvalForm.Create(Application);
  with Result do
  begin
    if ACaption<>'' then Caption:=ACaption;
    if Assigned(ADefines) then Defines:=ADefines
...
begin
  ...
  case Symbol.SymbolType of
    stValue      : Item.SubItems.Add(FloatToStr(TSymbolValue(Defines.Objects[Item.Index]).Value));
    stExpression : Item.SubItems.Add(TSymbolExpression(Defines.Objects[Item.Index]).Expression);
    stFunction   : if Symbol is TStandardFunction then with TStandardFunction(Symbol) do
                   begin
