...
uses
  ...
  {$IFDEF UseStyleForm} XPMan, {$ENDIF}
  Menus, ShellAPI, Comctrls, FileUtils, SysConst, Themes, ImgList, Buttons, FastCodePatch;
...
uses Types, Registry, StrUtils, TranslationTools, StringUtils, VersionInfo,
  StringLists, EventUtils, WinAPIUtils, EMailUtils;
...
begin
  ...
  if not IsLibrary and WindowsIsVistaOrLater and (Application.MainForm=nil) then
  begin
...
begin
  ...
  if not Scaled and (Font.Charset=DEFAULT_CHARSET) and (Screen.PixelsPerInch<>96) then // Avoid font scaling
    Font.Name:='Arial';
  if ParentFont and Assigned(Application.MainForm) then Font.Charset:=Application.MainForm.Font.Charset;
...
begin
  ...
  if FParentHandle<>0 then Params.WndParent:=FParentHandle;
end;
...
begin
  ...
  Position:=poDesigned;
  Left:=EnsureRange(Pos.X,Screen.WorkAreaLeft,Screen.WorkAreaWidth-Width);
  Top:=EnsureRange(Pos.Y,Screen.WorkAreaTop,Screen.WorkAreaHeight-Height);
end;
...
begin
  ...
  case P.ExceptionCode of
    STATUS_STACK_OVERFLOW :
      begin
        asm
          mov ESP, SafeSP // Try to restore a sane stack pointer
        end;
        MessageBox(Application.Handle,'Stack overflow.'#13'The application will terminate.',nil,MB_ICONERROR);
        Application.Terminate; // We can't hope to survive a stack overflow...
      end;
    ...
    begin
      case MessageDlgStr(Msg,mtError,[SMsgDlgIgnore,rsSendBugReport,rsTerminateApplication],1) of
        2 : begin
              ...
              try
                BugReport.values['to']:='whocares';
                BugReport.values['subject'] := 'Diagram Designer Bug report';
                BugReport.values['body'] := Msg;
                SendEMail(Application.Handle, BugReport);
              finally
                BugReport.Free;
              end;
            end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        SetupSaveDialogFilter(Dialog,FileName);
        Dialog.InitialDir:=ExtractFilePath(FileName);
      end;
    end;
    Result:=Dialog.Execute;
