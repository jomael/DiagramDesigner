...
uses
  ...
  Buttons, PanelFrame, DiagramBase;
...
begin
  ...
  if FloatQuery(rsEditLinkPoint,rsHorizontal+' (0-1):',X,0,1) and
     FloatQuery(rsEditLinkPoint,rsVertical+' (0-1):',Y,0,1) then
  begin
    if I=-1 then
    begin
...
begin
  ...
  begin
    ...
    CanvasInfo.Offset.X:=Round(1-Obj.Position.Left*CanvasInfo.Scale.X);
    CanvasInfo.Offset.Y:=Round(1-Obj.Position.Top*CanvasInfo.Scale.Y);
  ...
  if ListBox.ItemIndex>=0 then with SampleFrame.BitmapCanvas do
  begin
    Brush.Color:=clRed;
    Brush.Style:=bsSolid;
