...
type
  ...
  TValueEdit = class(TCustomEdit)
    protected
      ...
      //ParentFormClientProc : TClientProc;
...
{$IFDEF UseStyleForm}
function IntegerQuery(const ACaption, APrompt: string; var Value: Integer; Min, Max: Integer; DialogWidth: Integer=120; SpinIncrement: Integer=0): Boolean;
...
{type
  TValueEditList = class(TDynamicObjectList)
  private
    List : PPointerList;
    function Get(Index: Integer): TValueEdit;
  public
    constructor Create;
    property Items[Index: Integer]: TValueEdit read Get; default;
  end;

var
  ValueEditList : TValueEditList;

constructor TValueEditList.Create;
begin
  inherited Create(List);
  OwnsObjects:=False;
end;

function TValueEditList.Get(Index: Integer): TValueEdit;
begin
  Assert((Index>=0) and (Index<Count));
  Result:=TValueEdit(List^[Index]);
end;

// Process mouse wheel messages to see if we should change focus to a value edit control
function FormClientProc(Handle:HWND; Msg:UINT; wParam:WPARAM; lParam:LPARAM): LRESULT; stdcall;
var
  I : Integer;
  Sender : TValueEdit;
  MousePosScreen, MousePosClient : TPoint;
begin
  // Find form to which the message was sent
  Sender:=nil;
  for I:=0 to ValueEditList.Count-1 do
    if ValueEditList[I].ParentFormHandle=Handle then
    begin
      Sender:=ValueEditList[I];
      Break;
    end;
  if Sender=nil then
  begin
  ...
  begin
    MousePosScreen.X:=TSmallPoint(lParam).x;
    MousePosScreen.Y:=TSmallPoint(lParam).y;
    for I:=0 to ValueEditList.Count-1 do
      if ValueEditList[I].ParentFormHandle=Handle then
      begin
        MousePosClient:=ValueEditList[I].ScreenToClient(MousePosScreen);
        if (MousePosClient.X>=0) and (MousePosClient.Y>=0) and
           (MousePosClient.X<ValueEditList[I].Width) and (MousePosClient.Y<ValueEditList[I].Height) and
           ValueEditList[I].CanFocus then
        begin
          if not ValueEditList[I].Focused then // Mouse over unfocused value edit control, focus it
          begin
            ValueEditList[I].SetFocus;
            Result:=ValueEditList[I].Perform(Msg,wParam,lParam);
            Exit;
          end;
          Break;
        end;
  ...
  // Run old message handler
  Result:=Sender.ParentFormClientProc(Handle,Msg,wParam,lParam);
end;

// Remove value edit control from global list and restore parent form client proc
procedure RemoveFormClientProc(Control: TValueEdit);
var
  I : Integer;
  Found : Boolean;
begin
  if ValueEditList<>nil then
  begin
    // Remove control from global list of value edits
    Found:=False;
    for I:=0 to ValueEditList.Count-1 do
      if ValueEditList[I]=Control then
      begin
        ValueEditList.FastDelete(I);
        Found:=True;
        Break;
      end;
    // Restore parent form client proc if this was the last value edit on the form
    if Found and Assigned(Control.ParentFormClientProc) then
    begin
      Found:=False;
      for I:=0 to ValueEditList.Count-1 do
        if Control.ParentFormHandle=ValueEditList[I].ParentFormHandle then
        begin
          Found:=True;
          Break
        end;
      if not Found then
        SetWindowLong(Control.ParentFormHandle,GWL_WNDPROC,Integer(@Control.ParentFormClientProc));
    end;
    if ValueEditList.Count=0 then
      FreeAndNil(ValueEditList);
  end;
  Control.ParentForm:=nil;
  Control.ParentFormHandle:=0;
  Control.ParentFormClientProc:=nil;
end;}
...
begin
  ...
  {Control.ParentFormHandle:=Form.Handle;

  // Add to global list of value controls
  if ValueEditList=nil then
    ValueEditList:=TValueEditList.Create;
  ValueEditList.Add(Control);
  for I:=0 to ValueEditList.Count-2 do
    if ValueEditList[I].ParentFormHandle=Control.ParentFormHandle then
    begin
      // Another vlaue edit already exists on the same form
      Control.ParentFormClientProc:=ValueEditList[I].ParentFormClientProc;
      Exit;
    end;
  // Set form client proc
  Control.ParentFormClientProc:=TClientProc(SetWindowLong(Control.ParentFormHandle,GWL_WNDPROC,Integer(@FormClientProc)));}
end;
...
{$IFDEF UseStyleForm}
function IntegerQuery(const ACaption, APrompt: string; var Value: Integer; Min, Max,DialogWidth,SpinIncrement: Integer): Boolean;
var
  Form: TStyleForm;
  Prompt: TLabel;
  Edit: TIntegerEdit;
