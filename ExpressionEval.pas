...
uses Classes, SysUtils, StringLists, Math;
...
type
  ...
  TSymbolTable = class(TStringObjectList)
    public
      ...
      // Get symbol by name
      function Symbol(const Name: string): TSymbol;
  ...
  TSymbolValue = class(TSymbol)
    private
      FValue : EvalFloat;
    public
      constructor Create(const Value: EvalFloat=0);
  ...
  TSymbolExpression = class(TSymbol)
    private
      ...
      Active : Boolean;
    public
      constructor Create(const Expression: string; Pos: Integer);
      function Evaluate(Table: TSymbolTable): EvalFloat; virtual;
      property Expression: string read FExpression write FExpression;
    end;

  TStandardFunction = class(TSymbolFunction)
    private
      F : TFloatFunction;
      FDescription : string;
    public
...
var
      ...
      begin
        ...
        case Stack[StackTop].Operator of
          ...
          '*' : try
                  ...
                  on Error: EOverflow do if (V1^>0)=(V2^>0) then V1^:=Infinity else V1^:=NegInfinity;
                  on Error: Exception do raise EExpressionError.Create(Error.Message,Pos);
                end;
          '/' : try
                  V1^:=V1^/V2^;
                except
                  on Error: EZeroDivide do if V1^>0 then V1^:=Infinity else V1^:=NegInfinity;
                  on Error: EOverflow do if (V1^>0)=(V2^>0) then V1^:=Infinity else V1^:=NegInfinity;
                  on Error: Exception do raise EExpressionError.Create(Error.Message,Pos);
                end;
          '%' : try
          ...
          '^' : try
                  ...
                  on Error: EZeroDivide do V1^:=1/0;
                  on Error: EOverflow do
                  begin
                    if (V1^>0) and (V2^>0) then V1^:=Infinity
                    else if (V1^<0) and (V2^>0) then V1^:=NegInfinity
                    else V1^:=0
                  end;
                  on Error: Exception do raise EExpressionError.Create(Error.Message,Pos);
                end;
  ...
  begin
    ...
    repeat
      ...
      begin
        if StackTop<>0 then raise EExpressionError.Create(rsOperatorExpected,Pos);
      end
      else
      begin
        Operator:=Expression[Pos];
        if (StackTop=0) and not (Operator in ['+','-']) then raise EExpressionError.Create(rsUnexpectedCharacter+' '+Expression[Pos],Pos);
        Inc(Pos);
        SkipSeperators;
      end;
...
begin
  ...
  Pos:=1;
  OldMask:=SetExceptionMask([exDenormalized, exZeroDivide, exOverflow, exUnderflow, exPrecision]);
  try
    Result:=Evaluate;
  finally
...
function TSymbolTable.Symbol(const Name: string): TSymbol;
var
  I : Integer;
begin
  I:=IndexOf(Name);
...
begin
  I:=IndexOf(Name);
  if I<0 then raise Exception.Create('Symbol not found');
  Delete(I);
end;
